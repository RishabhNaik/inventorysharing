...
def register():
    ...
    #     function_abi = contract_instance.functions.registerUser('my_username', 0).transact({'from': user_address})
    #     function_call = web3.eth.account.signTransaction(function_abi, account.privateKey)
...
@app.route('/session_change')
def session_change():
    if session[user]=='gagan':
        session[user]='gagandeep'
    else:
        session[user]='gagan'    
